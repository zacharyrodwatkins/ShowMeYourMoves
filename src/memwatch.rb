...
class Memreader
  ...
  def initialize
    @@state = {}
    @@path = "#{ENV["HOME"]}/.local/share/dolphin-emu/"
    begin
      create_socket
    rescue => e
  ...
  def create_socket
    ...
    if ARGV[0]
      path = ARGV[0] + "MemoryWatcher/MemoryWatcher.txt"
    else
      path = @@path + "MemoryWatcher/MemoryWatcher.txt"
    end
  ...
  def update_address_value(address, value)
    case address
      when "80000000" then puts hex_to_float(value) 
